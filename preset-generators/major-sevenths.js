...
function majorSevenths() {
  const baseMap = [];
  const chordNames = ["I7", "ii7", "iii7", "IV7", "V7", "vi7", "vii7 (dim)"];
  const scaleNotes = Key.majorKey("C").scale;
  const notes = [
    ...scaleNotes.map((n) => Midi.toMidi(`${n}1`)),
    ...scaleNotes.map((n) => Midi.toMidi(`${n}2`)),
    ...scaleNotes.map((n) => Midi.toMidi(`${n}3`)),
  ];

  for (let i = 0; i < 7; i++) {
    baseMap.push({
      trigger: notes[i],
      notes: [notes[i], notes[i + 2], notes[i + 4], notes[i + 6]],
      name: chordNames[i],
    });
  }
  ...
  for (let i = 0; i < allNotes.length; i++) {
    ...
    writePreset(outDirPath, `${allNotes[i]}-major-sevenths`, xml);
  }
}

module.exports = { majorSevenths };
